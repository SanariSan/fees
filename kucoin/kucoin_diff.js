...
const writeFeesAndSymbolsToFile = async (fees) => {
  ...
  await fs.writeFile(
    "kucoin/kucoin_unique_maker_fees.json",
    JSON.stringify(fees.makerFees, null, 2)
  );
  console.log(
    "Unique maker fees and their symbols have been written to kucoin_unique_maker_fees.json"
  );

  // Writing taker fees and symbols to file
  await fs.writeFile(
    "kucoin/kucoin_unique_taker_fees.json",
    JSON.stringify(fees.takerFees, null, 2)
  );
  console.log(
    "Unique taker fees and their symbols have been written to kucoin_unique_taker_fees.json"
  );
};

// Main execution function
const main = async () => {
  const rawData = await fs.readFile("kucoin/kucoin_commissions.json", "utf8"); // Ensure the filename matches your actual file
